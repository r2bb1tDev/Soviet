...
Также клиент ведёт **локальную очередь (outbox)**: если адресат сейчас оффлайн или транспорт недоступен, сообщение сохраняется в SQLite и автоматически отправляется позже (offline messages как в ICQ).
...
**Правило авторизации (ICQ-style):**\n+- До принятия запроса **нельзя** отправлять личные сообщения\n+- После принятия контакт появляется в списке и можно писать
