    ...
    >
      ...
      <div style={s.header}>
        ...
        <div style={s.headerActions}>
          ...
          {activeChat?.id !== undefined && activeChat.id > 0 && !window.location.search.includes('chat=') && (
            <button
              className="btn-icon"
              title="Открепить чат в отдельное окно"
              onClick={async () => {
                try {
                  await invoke('open_chat_window', { chatId: activeChat.id })
                } catch (e) {
                  console.warn('[popout] open failed', e)
                }
              }}
            >
